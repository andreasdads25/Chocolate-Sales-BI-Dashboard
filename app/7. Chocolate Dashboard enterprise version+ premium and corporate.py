...
# ---------------------------------------------------------
st.set_page_config(
    page_title="Chocolate Sales Dashboard — ENTERPRISE",
    layout="wide",
...
def build_bi_pdf(df, filtered_df, drill_df, year_filter, country_filter, drill_country):
    ...
    H1 = styles["Heading1"]
    H1.textColor = CHARCOAL
    H1.fontSize = 18
    H1.spaceAfter = 10

    H2 = styles["Heading2"]
    H2.textColor = DARK_GREY
    H2.fontSize = 14
    H2.spaceAfter = 6
    ...
    story.append(Paragraph(f"<b>Year:</b> {year_filter}", Body))
    story.append(Paragraph(f"<b>Countries:</b> {', '.join(country_filter)}", Body))
    ...
    for line in insights:
        story.append(Paragraph(f"<font color='{GREEN}'>■</font> {line}", Body))
...
st.download_button(
    ...
    data=pdf_bytes,
    file_name=f"Chocolate_Sales_BI_Report_{year_filter}_StyleC.pdf",
